...
class MessageStore:
    ...
    def _migrate(self):
        ...
        with self._lock:
            ...
            if "status" not in agent_cols:
                self._conn.execute("ALTER TABLE agents ADD COLUMN status TEXT DEFAULT 'online'")
            if "working_on" not in agent_cols:
                self._conn.execute("ALTER TABLE agents ADD COLUMN working_on TEXT DEFAULT ''")

            cursor = self._conn.execute("PRAGMA table_info(messages)")
            msg_cols = {row[1] for row in cursor.fetchall()}
            if "msg_type" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN msg_type TEXT DEFAULT 'chat'")
            if "actor_id" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN actor_id TEXT")
            if "actor_type" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN actor_type TEXT DEFAULT 'agent'")
            if "target_id" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN target_id TEXT")
            if "target_type" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN target_type TEXT")
            if "event_type" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN event_type TEXT DEFAULT 'note.text'")
            ...
            if "labels" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN labels TEXT DEFAULT '[]'")
            if "mentions" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN mentions TEXT DEFAULT '[]'")
            if "artifacts" not in msg_cols:
                self._conn.execute("ALTER TABLE messages ADD COLUMN artifacts TEXT DEFAULT '[]'")
            self._conn.commit()
    ...
    def register_agent(self, name: str, role: str = "") -> Agent:
        ...
        with self._lock:
            self._conn.execute(
                "INSERT OR REPLACE INTO agents (name, role, status, working_on, connected_at, last_seen) VALUES (?, ?, ?, ?, ?, ?)",
                (agent.name, agent.role, agent.status, agent.working_on, agent.connected_at.isoformat(), agent.last_seen.isoformat()),
            )
            self._conn.commit()
        return agent
    ...
    def heartbeat(self, name: str, status: str = "online", working_on: str = "") -> Agent | None:
        ...
        with self._lock:
            ...
            row = self._conn.execute(
                "SELECT name, role, status, working_on, connected_at, last_seen FROM agents WHERE name = ?", (name,)
            ).fetchone()
        if not row:
            return None
        return Agent(
            name=row[0], role=row[1], status=row[2], working_on=row[3],
            connected_at=datetime.fromisoformat(row[4]), last_seen=datetime.fromisoformat(row[5]),
        )
    ...
    def list_agents(self) -> list[Agent]:
        ...
        with self._lock:
            rows = self._conn.execute(
                "SELECT name, role, status, working_on, connected_at, last_seen FROM agents ORDER BY last_seen DESC"
            ).fetchall()
        return [
            Agent(
                name=r[0], role=r[1], status=r[2] or "online", working_on=r[3] or "",
                connected_at=datetime.fromisoformat(r[4]), last_seen=datetime.fromisoformat(r[5]),
            )
            for r in rows
        ]
    ...
    def add_message(
        ...
        with self._lock:
            self._conn.execute(
                "INSERT INTO messages (id, actor_id, actor_type, target_id, target_type, event_type, metadata, labels, sender, recipient, thread, content, msg_type, mentions, artifacts, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
    ...
    def read_messages(
        ...
        query = "SELECT id, actor_id, actor_type, target_id, target_type, event_type, metadata, labels, sender, recipient, thread, content, msg_type, mentions, artifacts, timestamp FROM messages WHERE 1=1"
        params: list = []
        ...
        if as_agent:
            # Inbox: messages TO this agent OR broadcast (recipient IS NULL)
        ...
        return [
            Message(
                ...
                timestamp=datetime.fromisoformat(r[15]),
            )
